...
class Pmcod(c_query: Query) extends ProcessWindowFunction[(Int, Data_mcod), (Long, Query), Int, TimeWindow] {
  ...
  lazy val state: ValueState[PmcodState] = getRuntimeContext
    .getState(new ValueStateDescriptor[PmcodState]("myState", classOf[PmcodState]))
