...
class Explain(c_query: Query, c_distance: String, c_status: Int) extends ExplainOutlierDetection {
        ...
        if (p.mc == -1) {
          if (p.lSky.getOrElse(1, ListBuffer()).count(_._2 >= p.arrival) >= k_max) p.safe_inlier = true
            ...
            if (check_functionality(p, window_end)) {
              ...
              if (!rk_table(2)(2)) {
                explanation_count += 1
                val category = explain(rk_table, p.lSky.map(el => (el._1, el._2.size)), status, k)
