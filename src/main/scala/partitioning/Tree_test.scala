...
object Tree_test {
  ...
  class Tree_test_partitioning(c_range: Double, c_height: List[Int], c_sample: Array[Array[Double]], distance_type: String = "euclidean") extends Partitioning with Serializable {
    ...
    private val myHash = if (height > 0) createHash(c_sample) else null
    //Temp borders
    ...
    private def createHash(sample: Array[Array[Double]]): mutable.HashMap[MyId, MyNode] = {
      if (height == 0) null
      else {
        val delimiter = ";"
        //Create tree
        val my_vp_tree = createVPtree(sample)
        //Get thresholds
    ...
    private def createVPtree(sample: Array[Array[Double]]): VPTree[Data_basis, Data_basis] = {
      val finalSample = sample.toList.map(p => {
        new Data_basis(0, p.to[ListBuffer], 0L, 0)
      })
        .asJava
      new VPTree[Data_basis, Data_basis](myDistance, new SamplingMedianDistanceThresholdSelectionStrategy[Data_basis, Data_basis](finalSample.size()), finalSample)
    }
